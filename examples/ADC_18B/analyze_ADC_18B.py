"""Analyze ADC_18B: 18-bit SAR ADC step response."""
...
OUT  = HERE.parent.parent / 'output' / 'ADC_18B'

# 1. Simulate
evas_simulate(str(HERE / 'tb_ADC_18B.scs'), output_dir=str(OUT))

# 2. Load results
df = pd.read_csv(OUT / 'tran.csv')
t  = df['time'].values * 1e9  # → ns

# 3. Plot: clks | vin (step) | dout_code (step response)
fig, axes = plt.subplots(3, 1, figsize=(12, 7), sharex=True,
                         gridspec_kw={'height_ratios': [1.5, 2.5, 2.5]})

axes[0].plot(t, df['clks'], linewidth=1.0)
axes[0].set_ylabel('clks (V)')
axes[0].set_title('ADC_18B')
...
fig.tight_layout()
fig.savefig(str(OUT / 'analyze_ADC_18B.png'), dpi=150, bbox_inches='tight')
plt.close(fig)
print(f"Plot saved: {OUT / 'analyze_ADC_18B.png'}")
