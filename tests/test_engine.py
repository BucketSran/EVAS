...
"""
    ...
    def test_valueless_define_selects_voltage_behavior(self):
        src = """\
`include "disciplines.vams"
`define USE_HIGH
module pp_select_valueless(out);
    output voltage out;
    analog begin
`ifdef USE_HIGH
        V(out) <+ 1.2;
...
"""
    ...
    def test_laplace_nd_first_order_lowpass_advances_state(self):
        src = """\
`include "disciplines.vams"
module laplace_probe(vin, out);
    input voltage vin;
    output voltage out;
    analog begin
        V(out) <+ laplace_nd(V(vin), {1.0}, {1.0, 1.0});
    end
endmodule
"""
        ModelCls = compile_module(parse(src))
    ...
    def test_laplace_np_first_real_pole_advances_state(self):
        src = """\
`include "disciplines.vams"
module laplace_np_probe(vin, out);
    input voltage vin;
    output voltage out;
    analog begin
        V(out) <+ laplace_np(V(vin), {1.0}, {1.0, 1.0});
    end
endmodule
"""
        ModelCls = compile_module(parse(src))
...
"""
    ...
    def test_laplace_zp_first_real_pole_advances_state(self):
        src = """\
`include "disciplines.vams"
module laplace_zp_probe(vin, out);
    input voltage vin;
    output voltage out;
    analog begin
        V(out) <+ laplace_zp(V(vin), {1.0}, {1.0, 1.0});
    end
endmodule
"""
        ModelCls = compile_module(parse(src))
